            ...
            {filtered.map((project, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`project-card glass rounded-2xl p-6 border flex flex-col transition-all duration-300
                    ${isFeatured
                      ? 'border-amber-500/60 shadow-[0_0_30px_rgba(245,158,11,0.25)]'
                      : `${c.border} ${c.glow}`
                    }`}
                  style={{ boxShadow: isFeatured ? '0 0 30px rgba(245,158,11,0.15)' : 'none' }}
                >
                  {/* Header */}
                  ...
                  {project.highlight && (
                    <motion.div
                      animate={{ boxShadow: ['0 0 6px rgba(245,158,11,0.3)', '0 0 16px rgba(245,158,11,0.6)', '0 0 6px rgba(245,158,11,0.3)'] }}
                      transition={{ duration: 2, repeat: Infinity }}
                      className="mb-3 px-3 py-1 rounded-full text-xs font-bold text-amber-400 bg-amber-500/10 border border-amber-500/40 inline-flex items-center gap-1 w-fit"
                    >
                      {project.highlight}
                    </motion.div>
                  )}
                  ...
                  {/* Title & description */}
                  <h3 className={`font-bold text-lg mb-2 ${isFeatured ? 'text-amber-100' : 'text-white'}`}>{project.title}</h3>
