...
const projects = [
  ...
  },
  {
    ...
    category: 'AI/ML',
    color: 'purple',
    highlight: '🏆 National Hackathon Winner',
    github: 'https://github.com/Somicool',
  },
  {
    id: 3,
            ...
            {filtered.map((project, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`project-card glass rounded-2xl p-6 border ${c.border} ${c.glow} transition-all duration-300 flex flex-col`}
                  style={{ boxShadow: 'none' }}
                >
                  {/* Header */}
                  <div className="flex items-start justify-between mb-4">
                    <div className={`w-12 h-12 rounded-xl glass border ${c.border} flex items-center justify-center text-2xl`}>
                      {project.emoji}
                    </div>
                    <a
                      href={project.github}
                      ...
                      className="w-9 h-9 rounded-lg glass border border-white/10 flex items-center justify-center text-slate-400 hover:text-cyan-400 transition-colors"
                    >
                  ...
                  {/* Highlight badge */}
                  {project.highlight && (
                    <div className="mb-3 px-3 py-1 rounded-full text-xs font-bold text-amber-400 bg-amber-500/10 border border-amber-500/30 inline-flex items-center gap-1 w-fit">
                      {project.highlight}
                    </div>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {project.tags.map((tag) => (
                      <span key={tag} className={`px-2 py-1 rounded-md text-xs border font-mono ${c.tag}`}>
                        {tag}
                      </span>
                    ))}
                  </div>
